...
#include "InitDestBean_Wrapper.h"

/**
 * Wrapper methods for InitDestBean
 */
...
int InitDestBean_Wrapper::execVoidMethod(string& method, void** Prams, int num)
{
	if( !method.compare("initialize") && num == 0 )
		this->pBean->initialize();
	else if( !method.compare("destroy") && num == 0 )
		this->pBean->destroy();
	else 
		return -1;
...
int InitDestBean_Wrapper::getParamTypes(string& method, string& types, int num)
{
	if( !method.compare("InitDestBean") && num == 1 )
		types = string("")
			  + "SingletonBean*|";
	else if( !method.compare("initialize") && num == 0 )
		types = string("");
	else if( !method.compare("destroy") && num == 0 )
		types = string("");
...
AUTUMN_InitDestBean_Wrapper_Local _AUTUMN_InitDestBean_Wrapper_Local_;
